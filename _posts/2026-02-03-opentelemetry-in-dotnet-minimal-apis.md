...
*Update: Since writing this I've also been running Grafana Alloy as the collector both in the home lab and pointing at the work dev environment. Worth noting that Alloy's OTLP endpoint makes the local-to-cloud handoff nearly transparent — the instrumentation code doesn't change; only the OTLP exporter endpoint in `appsettings.Development.json` differs. That's the composability the spec promises, and it actually holds.*
