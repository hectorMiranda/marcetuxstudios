...
*Update: one thing I left out — health check dependencies need their own timeout configured, separate from the underlying client's timeout. If the SQL readiness check inherits a 30-second connection timeout, a slow SQL server stalls the probe long enough for Kubernetes to think the pod is unresponsive. We set a 5-second cancellation token on each dependency check so a probe that's waiting on a slow dependency fails fast and returns Degraded instead of hanging. Small config detail with a big operational difference.*
